...
class MultiScaleSegmenter:
    ...
    def prepare_document(self, text: str):
        ...
        self.token_embeddings = outputs.hidden_states[-1][0].detach()  # Remove batch dim
    ...
    def get_register_probs(self, text: str, start_char: int = None, end_char: int = None) -> np.ndarray:
    ...
    def compute_register_distinctness(self, probs1: np.ndarray, probs2: np.ndarray) -> float:
    ...
    def evaluate_split_individual(self, left_sents: List[str], right_sents: List[str]) -> float:
    ...
    def evaluate_split_pairs(self, left_sents: List[str], right_sents: List[str]) -> float:
        """Evaluate split using sentence pairs."""
        left_pairs = [" ".join(left_sents[i:i+2]) for i in range(0, len(left_sents)-1, 2)]
        right_pairs = [" ".join(right_sents[i:i+2]) for i in range(0, len(right_sents)-1, 2)]
    ...
    def evaluate_split_whole(self, left_sents: List[str], right_sents: List[str]) -> float:
    ...
    def find_best_split(self, sentences: List[str]) -> Tuple[int, float]:
        ...
        for i in range(self.config.min_sentences, len(sentences) - self.config.min_sentences + 1):
    ...
    def segment_recursive(self, text: str, sentences: List[str], 
                          sent_spans: List[Tuple[int, int]]) -> List[Tuple[str, np.ndarray]]:
        ...
        left_segments = self.segment_recursive(text, sentences[:split_idx], sent_spans[:split_idx])
        right_segments = self.segment_recursive(text, sentences[split_idx:], sent_spans[split_idx:])
    ...
    def truncate_text(self, text: str) -> str:
        ...
        if len(tokens) > self.config.max_length:
            text = self.tokenizer.decode(tokens[:self.config.max_length], skip_special_tokens=True)
        return text
